...
app.get('/floorplan', function(request, response) {
	response.sendFile('floorplan.html', { root: path.join(__dirname, './views') });
});

/** EMAIL US **/
app.get('/emailus', function(request, response) {
	response.sendFile('emailus.html', { root: path.join(__dirname, './views') });
});

/** MAP **/
app.get('/map', function(request, response) {
	response.sendFile('map.html', { root: path.join(__dirname, './views') });
});

/** FAQs **/
app.get('/faqs', function(request, response) {
	response.sendFile('faqs.html', { root: path.join(__dirname, './views') });
});

/** KIDS **/
app.get('/kids', function(request, response) {
	response.sendFile('kids.html', { root: path.join(__dirname, './views') });
});

/** DANCE **/
app.get('/dance', function(request, response) {
	response.sendFile('dance.html', { root: path.join(__dirname, './views') });
});

/** WEDDING **/
app.get('/wedding', function(request, response) {
	response.sendFile('wedding.html', { root: path.join(__dirname, './views') });
});
